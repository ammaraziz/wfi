...
if mamba create -n wfi --yes python=3.6 r-forge snakemake-minimal $GIT $DEPEND cutadapt; then
	...
	oops 'I cannot install snakemake in base environment. Attempting alternative method...'

	VPIPEENV=wfi
	# HACK Alternate to consider if we have have version conflicts
	conda create --yes -n ${VPIPEENV} -c conda-forge -c bioconda -c r snakemake-minimal biopython $DEPEND cutadapt conda git || fail "I cannot install snakemake in environment ${VPIPEENV}."
...
title 'Checking wf Installation'
...
if [[ -x $(which conda) ]] && conda --version 2>/dev/null >/dev/null; then # most computers with development tools installed, clusters, etc
	...
	oops 'conda is missing, thats really bad'
	CONDA=0
fi;
